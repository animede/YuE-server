...
from common import BlockTokenRangeProcessor, get_cache_class
...
def load_optimized_model(model_path, quantization, attention, use_mmgp=False, compilex=False):
    bnb_config = None
    ...
    if use_mmgp:
        return model
    #compileはうまく動かないのでコメントアウト
    #if torch.__version__ >= "2.0.0":
    #    print("Compile model")
    #    model = torch.compile(model, mode="max-autotune")
    #    return torch.compile(model, mode="max-autotune") if compilex else model
    return model
...
class SampleSettings:
    ...
    def __init__(self, use_guidance: bool = True, repetition_penalty: float = 1.1, do_sample=True):
        if not use_guidance:
            self.guidance_scale_seg0 = None
            self.guidance_scale = None
        self.repetition_penalty = repetition_penalty
        self.do_sample = do_sample  # 追加
...
class Stage1Pipeline_HF(Stage1Pipeline):
    def __init__(self,args, model_path: str, device: torch.device, cache_size: int, **kwargs):
        super().__init__(device, **kwargs)
        print("****Stage1Pipeline_HF")
        seed=args.seed
        ...
        compilex = True
        # Load HF model
        self.model = load_optimized_model(model_path, quantization_stage1, attn_implementation, use_mmgp, compilex)
        self.model.eval()
        if torch.__version__ >= "2.0.0":
            print("Compile model")
    ...
    def generate(
        ...
        # Tips: genre tags support instrumental，genre，mood，vocal timbr and vocal gender all kinds of tags are needed
        ...
        for i, p in enumerate(tqdm(prompt_texts[:run_n_segments], desc="Stage1 inference...")):
            ...
            with torch.no_grad():
                ...
                if output_seq[0][-1].item() != self.mmtokenizer.eoa:
                    tensor_eoa = torch.as_tensor([[self.mmtokenizer.eoa]]).to(self.model.device)
...
class Stage1Pipeline_EXL2(Stage1Pipeline):
    ...
    def generate(
        ...
        seed=None  # generatorの代わりにseedを直接受け取る
    ) -> torch.Tensor:
        global torch
        print("Stage1Pipeline_EXL2")
        if seed is not None:
            torch.manual_seed(seed)
        ...
        # 乱数生成器を初期化（シードを適用）
        if seed is not None:
            rng = random.Random(seed)  # Pythonの標準乱数生成器にもシードを設定
        else:
            rng = random.Random()
...
def stage1(args):#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< maineから関数に変更
    ...
    if args.stage1_use_exl2==True:
        ...
        raw_output = pipeline.generate(
                ...
                sample_settings=SampleSettings(use_guidance=not args.stage1_no_guidance, repetition_penalty=args.repetition_penalty,do_sample=False),
                seed=args.seed,
                )
    else:
    ...
    if not args.disable_offload_model:
        if args.stage1_use_exl2==True:
            ...
            pipeline.model.free_device_context()
        del pipeline
